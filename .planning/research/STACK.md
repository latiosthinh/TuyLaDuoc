...
**Domain:** Vietnamese consumer web app (Next.js) + small admin CMS, Turso/libSQL DB, anonymous visitors
**Researched:** 2026-09-13 — every version below verified TODAY against `registry.npmjs.org` (`npm view <pkg> version` / dist-tags) or official docs (Turso limitations + pricing pages, Vercel pricing page fetched via curl)
**Confidence:** HIGH (versions = registry-verified; free-tier numbers = official-pages-verified)

## Recommended Stack

One-liner: **Next.js 16.3.5 (App Router) + React 19.3.0 + Tailwind CSS 4.3.3 + Drizzle ORM 0.45.2 on Turso (@libsql/client 0.18.0) + Better Auth 1.7.4 (admin-only email/password) + Motion 13.2.0 + shadcn/ui (CLI 4.21.0) for the admin surface + Vercel Blob for images + Vitest 5 / Playwright 1.63 + pnpm 12.**
...
**Strategy: `next/font/google` — built into Next 16, zero extra packages.** (HIGH — stable Next feature since 13.)
...
const beVietnamPro = Be_Vietnam_Pro({ subsets: ["vietnamese", "latin"], variable: "--font-bvp" });
```
...
**Decision: DOM/SVG + Motion. No canvas. No GSAP. No physics engine.** (HIGH for the pattern, MEDIUM for "GSAP unnecessary" — judgment call.)
...
**Use Turso (primary). Verified free plan today (docs.turso.tech + turso.tech/pricing):**
...
App fit: dish library + content = a few hundred rows, <10MB. Reads: every page view hits a couple of queries → 500M/mo is ~1000x headroom. Writes: 1 per spin → 10M/mo ≈ 330K spins/day before block. Fine; add Vercel edge caching / `revalidateTag` for dish data so reads barely touch Turso.

**Turso-specific gotchas (verified on docs.turso.tech/cloud/limitations today):**
...
**What would force the Neon fallback:**
1. Need Postgres-only features: `pg_trgm`/real full-text search, pgvector, complex analytical joins (admin stats beyond counters).
2. Hard-block-at-quota unacceptable and you don't want Turso paid overages (Neon free plan auto-suspends compute instead of blocking storage — different failure mode).
3. Team later standardizes on Prisma/Postgres tooling.
...
pnpm add next@16.3.5 react@19.3.0 react-dom@19.3.0 \
  drizzle-orm@0.45.2 @libsql/client@0.18.0 better-auth@1.7.4 zod@4.6.4 \
  motion@13.2.0 @vercel/blob@2.8.0 tailwind-merge@3.7.0 clsx@2.1.1

# Dev deps:
pnpm add -D typescript@5.9.3 drizzle-kit@0.31.10 tsx@4.23.13 \
  @types/react@19.3.0 @types/react-dom@19.3.0 @types/node@22.20.2 \
  vitest@5.0.0 @vitejs/plugin-react@6.1.1 @testing-library/react@16.3.3 jsdom@30.0.1 \
  @playwright/test@1.63.0 prettier@3.9.6 typescript-eslint@8.70.0

# Admin UI (interactive, adds only what's used):
pnpm dlx shadcn@latest init
pnpm dlx shadcn@latest add button card input form table dialog select badge dropdown-menu tabs

# Optional stats:
pnpm add @vercel/analytics@2.0.1 @vercel/speed-insights@2.0.0
```

Node >=20.9 required (next engines, verified); use Node 22 LTS locally (Vercel runtime-compatible).
...
**If Turso hard-blocks become a production risk (viral traffic spike on writes):**
...
**If admin grows beyond ~10 entities / needs rich text + media library:**
...
**If "Ngẫu hứng" mode + 3 spin modes prove heavy on mobile:**
...
**If Vietnamese SEO becomes a growth lever:**
...
All fetched/verified **2026-09-13**:
...
**Known gaps (flag for phase research, none block roadmap):**
...
---
*Stack research for: Vietnamese lunch-picker web app (Next.js + Turso + admin CMS)*
*Researched: 2026-09-13 — all versions registry-verified same day*
